...
func renderServicesCustom(services []*descriptor.Service, paths swaggerPathsObject, reg *descriptor.Registry,
	requestResponseRefs, customRefs refMap) error {
	for svcIdx, svc := range services {
		for methIdx, meth := range svc.Methods {
			parameters := swaggerParametersObject{}
			var schema = makeSchema(meth.RequestType.FQMN(), reg, meth)
			var schemaRes = makeSchema(meth.ResponseType.FQMN(), reg, meth)
...
func makeSchema(fullyQuantifyMsgName string, reg *descriptor.Registry, meth *descriptor.Method) swaggerSchemaObject {
	var schema swaggerSchemaObject
	schema = swaggerSchemaObject{
		schemaCore: schemaCore{},
	}

	wknSchemaCore, isWkn := wktSchemas[fullyQuantifyMsgName]
	if !isWkn {
		schema.Ref = fmt.Sprintf("#/definitions/%s", fullyQualifiedNameToSwaggerName(fullyQuantifyMsgName, reg))
	} else {
...
func renderServices(services []*descriptor.Service, paths swaggerPathsObject, reg *descriptor.Registry,
	requestResponseRefs, customRefs refMap) error {
...
func applyTemplate(p param) (*swaggerObject, error) {
	...
	if err := renderServicesCustom(p.Services, s.Paths, p.reg, requestResponseRefs, customRefs); err != nil {
		panic(err)
	}
	...
	if spb != nil {
		...
		if spb.SecurityDefinitions != nil && spb.SecurityDefinitions.Security != nil {
			...
			for secDefKey, secDefValue := range spb.SecurityDefinitions.Security {
				...
				if secDefValue.Type != beeswaggeroptions.SecurityScheme_TYPE_INVALID {
					switch secDefValue.Type {
					case beeswaggeroptions.SecurityScheme_TYPE_BASIC:
						newSecDefValue.Type = "basic"
					case beeswaggeroptions.SecurityScheme_TYPE_API_KEY:
						newSecDefValue.Type = "apiKey"
					case beeswaggeroptions.SecurityScheme_TYPE_OAUTH2:
				...
				if secDefValue.In != beeswaggeroptions.SecurityScheme_IN_INVALID {
					switch secDefValue.In {
					case beeswaggeroptions.SecurityScheme_IN_QUERY:
						newSecDefValue.In = "query"
					case beeswaggeroptions.SecurityScheme_IN_HEADER:
						newSecDefValue.In = "header"
					}
				}
				if secDefValue.Flow != beeswaggeroptions.SecurityScheme_FLOW_INVALID {
					switch secDefValue.Flow {
					case beeswaggeroptions.SecurityScheme_FLOW_IMPLICIT:
						newSecDefValue.Flow = "implicit"
					case beeswaggeroptions.SecurityScheme_FLOW_PASSWORD:
						newSecDefValue.Flow = "password"
					case beeswaggeroptions.SecurityScheme_FLOW_APPLICATION:
						newSecDefValue.Flow = "application"
					case beeswaggeroptions.SecurityScheme_FLOW_ACCESS_CODE:
...
// beeswagger_options.Operation from a given proto method's descriptor.
func extractOperationOptionFromMethodDescriptor(meth *pbdescriptor.MethodDescriptorProto) (*beeswaggeroptions.Operation, error) {
	if meth.Options == nil {
		return nil, nil
	}
	if !proto.HasExtension(meth.Options, beeswaggeroptions.E_Openapiv2Operation) {
		return nil, nil
	}
	ext, err := proto.GetExtension(meth.Options, beeswaggeroptions.E_Openapiv2Operation)
	if err != nil {
		return nil, err
	}
	opts, ok := ext.(*beeswaggeroptions.Operation)
	if !ok {
...
// beeswagger_options.Schema from a given proto message's descriptor.
func extractSchemaOptionFromMessageDescriptor(msg *pbdescriptor.DescriptorProto) (*beeswaggeroptions.Schema, error) {
	if msg.Options == nil {
		return nil, nil
	}
	if !proto.HasExtension(msg.Options, beeswaggeroptions.E_Openapiv2Schema) {
		return nil, nil
	}
	ext, err := proto.GetExtension(msg.Options, beeswaggeroptions.E_Openapiv2Schema)
	if err != nil {
		return nil, err
	}
	opts, ok := ext.(*beeswaggeroptions.Schema)
	if !ok {
...
// beeswagger_options.Swagger from a given proto method's descriptor.
func extractSwaggerOptionFromFileDescriptor(file *pbdescriptor.FileDescriptorProto) (*beeswaggeroptions.Swagger, error) {
	if file.Options == nil {
		return nil, nil
	}
	if !proto.HasExtension(file.Options, beeswaggeroptions.E_Openapiv2Swagger) {
		return nil, nil
	}
	ext, err := proto.GetExtension(file.Options, beeswaggeroptions.E_Openapiv2Swagger)
	if err != nil {
		return nil, err
	}
	opts, ok := ext.(*beeswaggeroptions.Swagger)
	if !ok {
...
func extractJSONSchemaFromFieldDescriptor(fd *pbdescriptor.FieldDescriptorProto) (*beeswaggeroptions.JSONSchema, error) {
	if fd.Options == nil {
		return nil, nil
	}
	if !proto.HasExtension(fd.Options, beeswaggeroptions.E_Openapiv2Field) {
		return nil, nil
	}
	ext, err := proto.GetExtension(fd.Options, beeswaggeroptions.E_Openapiv2Field)
	if err != nil {
		return nil, err
	}
	opts, ok := ext.(*beeswaggeroptions.JSONSchema)
	if !ok {
...
func protoJSONSchemaToSwaggerSchemaCore(j *beeswaggeroptions.JSONSchema, reg *descriptor.Registry, refs refMap) schemaCore {
	ret := schemaCore{}
...
func updateSwaggerObjectFromJSONSchema(s *swaggerSchemaObject, j *beeswaggeroptions.JSONSchema) {
	s.Title = j.GetTitle()
...
func swaggerSchemaFromProtoSchema(s *beeswaggeroptions.Schema, reg *descriptor.Registry, refs refMap) swaggerSchemaObject {
	ret := swaggerSchemaObject{
...
func protoJSONSchemaTypeToFormat(in []beeswaggeroptions.JSONSchema_JSONSchemaSimpleTypes) (string, string) {
	if len(in) == 0 {
		return "", ""
	}
	...
	switch in[0] {
	case beeswaggeroptions.JSONSchema_UNKNOWN, beeswaggeroptions.JSONSchema_NULL:
		return "", ""
	case beeswaggeroptions.JSONSchema_OBJECT:
		return "object", ""
	case beeswaggeroptions.JSONSchema_ARRAY:
		return "array", ""
	case beeswaggeroptions.JSONSchema_BOOLEAN:
		return "boolean", "boolean"
	case beeswaggeroptions.JSONSchema_INTEGER:
		return "integer", "int32"
	case beeswaggeroptions.JSONSchema_NUMBER:
		return "number", "double"
	case beeswaggeroptions.JSONSchema_STRING:
		// NOTE: in swagger specifition, format should be empty on string type
		return "string", ""
	default:
		// Maybe panic?
...
func protoExternalDocumentationToSwaggerExternalDocumentation(in *beeswaggeroptions.ExternalDocumentation) *swaggerExternalDocumentationObject {
	if in == nil {
		return nil
	}
